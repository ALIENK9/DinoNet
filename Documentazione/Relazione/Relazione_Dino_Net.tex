...
\begin{document}
	...
	\begin{itemize}
		...
		Un possibile account non amministratore da utilizzare è il seguente:
		\begin{itemize}
			\item \textbf{Username:} piero@angela.it
	...
	\end{itemize}
	È inoltre sempre possibile aggiungere altri account dal pannello di amministrazione.\\
	È opportuno segnalare che non è possibile essere connessi contemporaneamente con due account, perché all'accesso di un utente vengono ripulite le variabili di sessione.	
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\end{itemize}
		\end{itemize}
		\item \textbf{Accedi (login.php)}: qui ogni utente può accedere al proprio account per scrivere i propri commenti agli articoli, oppure decidere di registrarne uno nuovo. Anche gli amministratori possono effettuare l'accesso da questa pagina per il loro account utente (non accederanno però al pannello di amministrazione, raggiungibile solo accedendo dalla pagina indicata all'inizio della relazione).
		\begin{itemize}
	...
	\end{itemize}
	Si noti che tutti i contenuti sono ordinati per data di inserimento.
	...
	Eventuali errori nella compilazione sono segnalati chiamando le apposite funzioni inserite in \code{message.php}. Per motivi estetici le immagini devono avere le dimensioni specificate nella label del relativo input, se così non fosse la funzione \code{imagescale()} le scalerà automaticamente alle dimensioni corrette.
	...
	Il sito si appoggia su database realizzato con \textit{MySQL}, di cui segue lo schema della struttura.
	\begin{figure}[h]
		\centering
		\includegraphics[width=\linewidth]{db.jpeg}
		\caption{Schema del database del sito}
		\label{fig:schemadb}
	\end{figure}\\
	Nello sviluppo del sito alcune scelte fatte per la realizzazione del database sono risultate non ottimali e hanno creato alcuni problemi. Ad esempio l'utilizzo del nome del dinosauro come chiave primaria dell'entità \textit{dinosauro} ha portato ad alcuni problemi con la codifica dei caratteri speciali presenti nella stringa del nome. Li abbiamo risolti attraverso l'utilizzo di alcune funzioni PHP come \code{htmlspecialchars()} e \code{mysqli::real\_escape\_string()}. In aggiunta in alcuni casi, la scelta di una chiave primaria diversa (ad esempio un intero) avrebbe reso più semplice implementare certe funzionalità aggiuntive. \\Una conseguenza di questa scelta è, ad esempio, l'impossibilità di modificare il nome di un dinosauro già inserito, oppure di modificare l'indirizzo email di un utente iscritto.\\	
	...
	Nella sezione dedicata all'amministratore talvolta è utile, dopo una richiesta dell'utente, chiedere di confermare l'azione, in modo da prevenire errori dovuti a click accidentali (ad esempio se ha chiesto di eliminare un account o un articolo). Per fare questo utilizziamo dei \textit{confirm box}, richiamati con la funzione \code{window.confirm()} di JavaScript. Nel caso l'esecuzione di script sia disabilitata, semplicemente non verrà chiesta tale conferma.
	...
	\begin{itemize}
		...
		\item \textbf{NVDA}: pur con la consapevolezza che la prova non sia esauriente, il sito è stato testato con lo screen reader NVDA e gli aiuti alla navigazione inseriti sono risultati propriamente funzionanti, tranne nel caso dell'attributo \code{aria-hidden}, il quale viene ignorato su Microsoft Edge e Firefox;
