...
\begin{document}
	...
	Il sito utilizza un foglio di stile principale (denominato \code{index.css}), che gestisce anche la responsiveness del sito per quanto riguarda i dispositivi mobile tramite media query, e uno per la stampa (denominato \code{print.css}). Nello sviluppo del foglio di stile si è cercato di progettare le classi in modo tale da minimizzarne il numero e massimizzarne il riutilizzo. Questa scelta mostra il pregio del conseguimento di un foglio di stile facilmente utilizzabile, riutilizzabile ed estendibile, ed il difetto portato dalla necessario appesantimento in termini di leggibilità del codice HTML che vedrà \code{div} a cui saranno attribuite molte classi. 
